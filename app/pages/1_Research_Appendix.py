"""Research appendix: documents the non-promoted satellite-calibration and LUR
modeling workstreams. Structurally separate from the default screening
workflow (app/streamlit_app.py) — Streamlit's classic pages/ mechanism means
this script only runs when a user deliberately navigates here, and it never
imports or renders anything from the main screening page.
"""
...
st.set_page_config(page_title="Research Appendix — WI Environmental Health Explorer", layout="wide")

st.title("Research Appendix: Modeling Workstream (Not Promoted)")
st.warning(RESEARCH_APPENDIX_WARNING)

st.header("1. Satellite Calibration Model")
st.markdown(
...
st.divider()
st.header("2. LUR Pilots: Madison PM2.5, Milwaukee NO2")
st.markdown(
...
def _render_pilot(city: str, pollutant: str, plot_prefix: str) -> None:
...
with madison_tab:
    _render_pilot("madison", "pm25", "madison_pm25")

with milwaukee_tab:
    _render_pilot("milwaukee", "no2", "milwaukee_no2")
...
st.divider()
st.header("What this project does not show")
st.markdown(
